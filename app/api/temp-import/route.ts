...
export async function GET(request: Request) {
  try {
    ...
    for (const rec of records) {
      ...
      if (existing) {
        // Accumulate values if CSV itself contains duplicate phone entries
    ...
    // 2. Fetch existing profiles to merge totals
    const { data: existingProfiles, error: profError } = await admin
      .from("customer_profiles")
      .select("phone, name, points_balance, total_spent, visit_count, created_at");
    ...
    // 3. Fetch existing finalized orders for NerfTurf to avoid duplicating them
    ...
    const existingPhonesForLoc = new Set(existingOrders?.map(o => o.customer_phone).filter(Boolean));

    // 4. Prepare inserts and upserts
    ...
    for (const rec of csvRecords) {
      ...
      // Only create order history if they spent money and don't already have finalized orders at NerfTurf
      if (rec.total_spent > 0 && !existingPhonesForLoc.has(rec.phone)) {
    ...
    // 5. Execute DB operations in batches
    const batchSize = 100;

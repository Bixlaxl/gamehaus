...
export default async function BookingsPage() {
  ...
  const { data: bookings } = await admin
    .from("bookings")
    .select(`*, order:orders(customer_name, customer_phone, advance_paid, type, status, subtotal, discount_amount, total_amount, points_redeemed, public_discount_amount, points_redeemed_online), order_item:order_items(table:tables(id, name, type, location:locations(name, id)))`)
